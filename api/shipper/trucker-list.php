...
*/
		...
		$sel = $Global->db->prepare("SELECT count(*) as allcount FROM favorite INNER JOIN trucker ON trucker.id = favorite.trucker_id INNER JOIN users ON users.id=trucker.user_id WHERE favorite.favorite_type='trucker_favorite' AND favorite.status=1 AND favorite.shipper_id=:shipper_id");
		$sel->execute(array("shipper_id"=>$shipper_id));
		$records =$sel->fetch(PDO::FETCH_ASSOC);
		$totalRecords = $records['allcount'];

		// Total number of record with filtering
		$sel = $Global->db->prepare("SELECT count(*) as allcount FROM  favorite INNER JOIN trucker ON trucker.id = favorite.trucker_id INNER JOIN users ON users.id=trucker.user_id WHERE (users.business_name ::text ILIKE :searchQuery OR  users.email ::text ILIKE :searchQuery  OR users.name ::text ILIKE :searchQuery OR trucker.vehicle_number ::text ILIKE :searchQuery OR trucker.phone ::text ILIKE :searchQuery) AND favorite.favorite_type='trucker_favorite' AND favorite.status=1 AND favorite.shipper_id=:shipper_id ");
		$sel->execute(array("searchQuery" => $searchQuery,"shipper_id"=>$shipper_id));
		$records =$sel->fetch(PDO::FETCH_ASSOC);
		$totalRecordwithFilter = $records['allcount'];

		// Fetch records
		$trucklists = $Global->db->prepare("SELECT users.business_name,users.email ,favorite.shipper_id,favorite.trucker_id,favorite.id,favorite.favorite_type,favorite.status,trucker.phone,trucker.vehicle_number as dot,users.name FROM  favorite INNER JOIN trucker ON trucker.id = favorite.trucker_id INNER JOIN users ON users.id=trucker.user_id  WHERE (users.business_name ::text ILIKE :searchQuery OR users.email ::text ILIKE :searchQuery OR users.name ::text ILIKE :searchQuery OR trucker.vehicle_number ::text ILIKE :searchQuery OR trucker.phone ::text ILIKE :searchQuery) AND favorite_type='trucker_favorite' AND favorite.status=1 AND favorite.shipper_id=:shipper_id   ORDER BY ".$columnName." ".$columnSortOrder."  LIMIT ".$rowperpage." OFFSET ".$row);
