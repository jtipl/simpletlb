...
if(empty($token)){
	...
	$stmt = $Global->db->prepare("SELECT id FROM favorite WHERE trucker_id=:trucker_id AND shipper_id=:shipper_id AND favorite_type='trucker_favorite' ");
	...
	if(empty($stmt_result)){
		...
		$aVars=array("status"=>1 , "msg"=>"Favorite successfully");
		    ...
		    $del_conditions =array("shipper_id"=>$shipper_id,"trucker_id"=>$trucker_id,"favorite_type"=>"trucker_favorite");
