...
class BrokerSignup extends LoadBoard
{
	public function Signup($arr=array()){
		...
		if(empty($name)){
			...
			$email = $this->db->prepare( "SELECT id, subject, content, email_notication FROM public.email_template WHERE type='shipper-verification-email'");
			$email->execute();
			...
			$aVars=array("status"=>1,"msg"=>"Registered Successfully","user_type"=>"shipper"); 	
