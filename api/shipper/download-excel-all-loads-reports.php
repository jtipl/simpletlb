...
FROM loads l
LEFT JOIN loads_trip ON loads_trip.load_id=l.id  
WHERE l.user_id=:user_id 
AND   l.pickup_date > CURRENT_DATE 
AND (l.origin ::text ILIKE :origin AND l.destination ::text ILIKE :destination  AND l.load_id ::text ILIKE :loadid AND l.weight ::text ILIKE :weight AND l.height ::text ILIKE :height AND l.length ::text ILIKE :length AND l.price ::text ILIKE :price $datequery) $eqquery  $status_qry ORDER by l.id DESC LIMIT ".$rowperpage." OFFSET ".$row;
//echo $sql;exit;
