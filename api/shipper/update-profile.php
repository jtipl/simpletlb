...
class ShipperUpdateProfile extends LoadBoard{

//$headers = apache_request_headers();
public function ShipperUpdateTimeProfile($arr=array()){
	...
	if(empty($token)){
		$aVars=array("status"=>0 , "msg"=>"Empty token");
			...
			$shipper_name=isset($arr['shipper_name']) ? trim($arr['shipper_name']) : '';
			$business=isset($arr['business_name']) ? trim($arr['business_name']) : '';
			$phone=isset($arr['shipper_phone']) ? trim($arr['shipper_phone']) : '';
			$address=isset($arr['shipper_addr']) ? trim($arr['shipper_addr']) : '';
			...
			$name_valid=parent::CharacterCheck($shipper_name);
			...
			if(empty($user_id)){
			...
			}elseif(empty($shipper_name)){
				$aVars=array("status"=>0,"msg"=>"Please enter the name");
			}elseif(!empty($shipper_name) && is_numeric($shipper_name)){
				$aVars=array("status"=>0,"msg"=>"Please enter a valid Name");
			}elseif(!empty($shipper_name) && $name_valid==false){
				$aVars=array("status"=>0,"msg"=>"Please enter a valid Name");
		     ...
		     if(!empty($success_data) && $success_data == 'valid' ){
							...
							parent::UpdateData("shipper",$datas,$conditions_user_id);
							...
							$token = array(
							    ...
							    'ICON'  => $imgicon,
							    'SITEURL'=> SITEURL,
							    'USER-NAME'=>ucfirst($shipper_name),
							    'INFO'=>INFO_EMAIL,
...
if(!empty($input)){
	$shipperupdatetime=new ShipperUpdateProfile();
	$shipperupdatetime->ShipperUpdateTimeProfile($input);
}
