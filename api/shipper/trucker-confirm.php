...
$trucker_email=$email;
$shipper_email=isset($_SESSION['email']) ? trim($_SESSION['email']) : '';
...
$Global->UpdateData("loads_trip",$datas,$conditions);
	...
	$shipper_id= isset($_SESSION['user_id']) ? trim($_SESSION['user_id']) : '';
	$shipper_det = $Global->db->prepare("SELECT users.business_name, shipper.phone FROM users LEFT JOIN shipper ON users.id = shipper.user_id WHERE users.status = :status AND users.id =:id AND shipper.user_id = :id");
	$shipper_det->execute(array("status"=>1,"id"=>$shipper_id));
	$shipper_dets = $shipper_det->fetch(PDO::FETCH_ASSOC);
	$shipper_busi_name = $shipper_dets['business_name'];
	$shipper_phone = $shipper_dets['phone'];
...
//shipper notication
	...
	$shipper_emails = $Global->db->prepare( "SELECT id, subject, content, email_notication
				FROM public.email_template WHERE type='thanks-confrim-mail-shipper'");
	$shipper_emails->execute();
	$shipper_email_template= $shipper_emails->fetch(PDO::FETCH_ASSOC);
	$shipper_subject=$shipper_email_template['subject'];
	$token = array(
	    'IMGLINK'  			=> $imglink,
	    'SITEURL'			=> SITEURL,
		'ICON'				=> $imgicon,
	...
	$varMap[sprintf($shipper_pattern,$key)] = $val;
	}
	$shipper_content = strtr($shipper_email_template['content'],$varMap);
	$Global->SendEmail(FROM_EMAIL,$shipper_email,$shipper_subject,$shipper_content);
...
//truker notification
 ...
 $trucker_token = array(
    'IMGLINK'  			=> $imglink,
    'SITEURL'			=> SITEURL,
	//'BUSINESS-NAME'		=> $shipper_busi_name,
	'ICON'				=> $imgicon,
	'BROKER-NAME'		=> ucfirst($_SESSION['name']),
	'TRUCKER-NAME'		=> ucfirst($truckuserdets['name']),
	'LOAD-ID'			=> $GetLoad['load_id'],
...
$Global->SendEmail($shipper_email,$trucker_email,$subject,$content);
//denied email
