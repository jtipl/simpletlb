...
if($CheckvalidToken['status']==1){
	...
	$cancel_sel = $Global->db->prepare("SELECT count(*) as allcount FROM loads_trip INNER JOIN loads ON loads.id = loads_trip.load_id WHERE loads_trip.broker_id=:broker_id AND loads_trip.cancel_status=2 ");//AND loads.status=2
	$cancel_sel->execute(array("broker_id"=>$broker_id));
	...
	$favouritesel = $Global->db->prepare("SELECT count(*) as allcount FROM favorite INNER JOIN trucker ON trucker.id = favorite.trucker_id INNER JOIN users ON users.id=trucker.user_id WHERE favorite.favorite_type='trucker_favorite' AND favorite.status=1 AND favorite.broker_id=:broker_id");
	$favouritesel->execute(array("broker_id"=>$broker_id));
