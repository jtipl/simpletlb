...
$objPHPExcel->getActiveSheet()->setCellValue('O1','LENGTH');
...
*/
//loadid
$loadid = isset($_REQUEST['trucker_load_id']) ? $_REQUEST['trucker_load_id']: '';
...
//echo $datequery;exit;
//print_r($_REQUEST['trucker_load_status']);

if($_REQUEST['trucker_load_status']!=""){
	$trucker_load_status=$_REQUEST['trucker_load_status'];
	if($trucker_load_status=="undefined"){
		$status_qry= "";
	} else {
		$status_qry= " AND loads_trip.load_status IN (".$trucker_load_status.")";;
	}
} else {
	$status_qry = "";	
}
...
OR users.name ::text ILIKE :trucker_name
OR users.business_name ::text ILIKE :business_name 
OR trucker.vehicle_number ::text ILIKE :vehicle_number
 ) $datequery $status_qry
 LIMIT ".$rowperpage." OFFSET ".$row;
//echo $sql;exit;
$query = $Global->db->prepare($sql);
$query->execute(array("user_id"=>$user_id,"loadid"=>$sloadid,"trucker_name"=>$strucker_name,"business_name"=>$sbusiness_name,"vehicle_number"=>$susdot));
