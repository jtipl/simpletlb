...
if($operation=="pending" || $operation==""){
	...
	$sql = "SELECT l.load_id , l.origin , l.destination ,l.origin_address , l.origin_state , l.origin_city ,  
		...
		$query = $Global->db->prepare($sql);
		$query->execute(array("user_id"=>$user_id,"tomarrow_date"=>$currDate['current_date']));
	...
	$sql = "SELECT l.load_id , l.origin , l.destination ,l.origin_address , l.origin_state , l.origin_city ,  
		...
		FROM loads l INNER JOIN  truck_type tt
		ON 
		l.status IN(1,5) AND l.approve_status=1  AND l.cancel_truckers <> l.confirm_truckers_id
		AND l.user_id=:user_id 
		AND l.truck_id = tt.id
		ORDER by l.id DESC LIMIT ".$rowperpage." OFFSET ".$row;
		*/
	...
	$sql = "
	SELECT loads.load_id , loads.origin , loads.destination ,loads.origin_address , loads.origin_state , loads.origin_city ,  
		...
		loads.truck_load_type , loads.weight , loads.length , loads.price,loads.height , loads.status , loads.truck_id , tt.truck_name
	...
	$title="Cancel_Loads";
		...
		$sql = "SELECT l.load_id , l.origin , l.destination ,l.origin_address , l.origin_state , l.origin_city ,  l.origin_zipcode , l.destination_address , l.destination_state , l.destination_city,
			...
			l.truck_load_type , l.weight , l.length , l.price,l.height , l.status ,l.truck_id , tt.truck_name
				...
				INNER JOIN users ON users.id=loads_trip.user_id 
				INNER JOIN  truck_type tt ON l.truck_id = tt.id
			 WHERE  loads_trip.broker_id=:broker_id AND loads_trip.cancel_status=2     LIMIT ".$rowperpage." OFFSET ".$row;
	$query = $Global->db->prepare($sql);
	...
	$sql = "SELECT 
		...
		WHERE loads.user_id=:user_id AND loads.status=0  AND loads.pickup_date < CURRENT_DATE  
