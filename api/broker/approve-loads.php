...
if(empty($user_id)){
		...
		$sel = $Global->db->prepare("SELECT count(*) as allcount  FROM loads_trip INNER JOIN loads ON loads.id=loads_trip.load_id INNER JOIN  trucker ON trucker.id=loads_trip.trucker_id INNER JOIN  users ON users.id=trucker.user_id WHERE loads_trip.broker_id=:broker_id AND loads.id=:loadid AND loads_trip.load_status =1 AND loads_trip.is_delete=0 AND loads_trip.denied_status=0 AND loads_trip.history_status=1    ");
