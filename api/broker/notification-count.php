...
if($CheckvalidToken['status']==1){
	...
	$loads_canceltrip = $Global->db->prepare("SELECT count(*) as count FROM loads l join loads_trip lt 
												on l.id=lt.load_id 
												and lt.broker_id=:broker_id and lt.cancel_status =1 AND l.broker_notification_view !='5'");
	*/
	//$loads_canceltrip = $Global->db->prepare("SELECT count(*) as count FROM loads l WHERE 
	

	$loads_canceltrip = $Global->db->prepare("SELECT count(*) as count FROM loads l WHERE 
												user_id=:user_id  AND status= '5' AND broker_notification_view IN(0,5)");
	...
	$loads_picked = $Global->db->prepare("SELECT count(*) as count FROM loads l join loads_trip lt 
							on l.id=lt.load_id and lt.broker_id=:broker_id and lt.load_status in (3) AND lt.broker_notification_view != '3'  AND lt.is_delete=0 ");
	...
	$loads_delivered = $Global->db->prepare("SELECT count(*) as count FROM loads l join loads_trip lt 
												on l.id=lt.load_id 
												and lt.broker_id=:broker_id and lt.load_status =4 AND lt.is_delete=0  AND  lt.broker_notification_view != '4'
												");
	$loads_delivered->execute(array("broker_id"=>$broker_id));
