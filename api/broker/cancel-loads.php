...
$Global=new LoadBoard();
	...
	if(empty($token)){
		...
		if(empty($user_id)){
			...
			$sel = $Global->db->prepare("SELECT count(*) as allcount FROM loads_trip INNER JOIN loads ON loads.id = loads_trip.load_id WHERE loads_trip.broker_id=:broker_id AND loads_trip.cancel_status=2 ");//AND loads.status=2
			$sel->execute(array("broker_id"=>$broker_id));
			$records =$sel->fetch(PDO::FETCH_ASSOC);
			$totalRecords = $records['allcount'];

			// Total number of record with filtering
			$sel = $Global->db->prepare("SELECT count(*) as allcount FROM loads_trip INNER JOIN loads ON loads.id = loads_trip.load_id WHERE (loads.origin ::text ILIKE :searchQuery) AND loads_trip.broker_id=:broker_id AND loads_trip.cancel_status=2 ");
			$sel->execute(array("searchQuery" => $searchQuery,"broker_id"=>$broker_id));
			$records =$sel->fetch(PDO::FETCH_ASSOC);
			$totalRecordwithFilter = $records['allcount'];

			// Fetch records
			$trucklists = $Global->db->prepare("SELECT loads_trip.trucker_id,loads_trip.cancel_reason, loads_trip.cancel_date, loads_trip.id as tripid,loads.price,loads.weight,loads.length,loads.load_id,loads.origin as origin,loads.destination,loads.id,loads.status as loadstatus,loads_trip.load_status as tripstatus,trucker.phone,trucker.vehicle_number as dot,users.name FROM loads_trip INNER JOIN loads ON loads.id = loads_trip.load_id INNER JOIN trucker ON trucker.id=loads_trip.trucker_id INNER JOIN users ON users.id=loads_trip.user_id  WHERE (loads.origin ::text ILIKE :searchQuery OR loads.destination ::text ILIKE :searchQuery OR loads.load_id ::text ILIKE :searchQuery OR loads.weight ::text ILIKE :searchQuery OR loads.length ::text ILIKE :searchQuery OR loads.price ::text ILIKE :searchQuery) AND loads_trip.broker_id=:broker_id AND loads_trip.cancel_status=2   ORDER BY ".$columnName." ".$columnSortOrder."  LIMIT ".$rowperpage." OFFSET ".$row);
			//AND loads.status=1
