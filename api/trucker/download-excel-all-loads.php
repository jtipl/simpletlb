...
if($operation=="awaiting" || $operation==""){
	...
	INNER JOIN  truck_type tt ON l.truck_id = tt.id
	WHERE 
	loads_trip.trucker_id=:trucker_id AND loads_trip.cancel_status IN (1,2)  
	LIMIT ".$rowperpage." OFFSET ".$row;
	//echo 
	$query = $Global->db->prepare($sql);
	$query->execute(array("trucker_id"=>$trucker_id));
	...
	WHERE 
	loads_trip.trucker_id=:trucker_id AND loads_trip.load_status=4  AND loads_trip.is_delete=0   LIMIT ".$rowperpage." OFFSET ".$row;
	//echo $sql;exit;
	$query = $Global->db->prepare($sql);
	$query->execute(array("trucker_id"=>$trucker_id));

} else if($operation=="recent_viewloads"){
	$title = "Recent_all_viewedloads";
	...
	INNER JOIN  viewlist ON l.id = viewlist.load_id
	WHERE
	viewlist.user_id=:user_id AND viewlist.status=1 LIMIT ".$rowperpage." OFFSET ".$row." ";
		//viewlist INNER JOIN loads ON loads.id = viewlist.load_id INNER JOIN truck_type as truck ON truck.id=loads.truck_id  
