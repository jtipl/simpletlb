...
class TruckerSignup extends LoadBoard
{
	public function Signup($arr=array()){
		...
		if(empty($name)){
			...
			$aVars=array("status"=>0,"msg"=>array("name"=>"Please enter a valid Name"));
		}
		...
		elseif(empty($business_fullname)){
			...
			$aVars=array("status"=>0,"msg"=>array("business_name"=>"Please enter a valid Business Name"));
		}
		...
		elseif(empty($business_email)){
			...
			$aVars=array("status"=>0,"msg"=>array("confirm_pass"=>"Password and confirm password mismatch"));
			...
			FROM public.email_template WHERE type='trucker-verification-email'");
			$email->execute();
			...
			$imgicon=SITEURL."app/assets/images/handshake.png";
			...
			$aVars=array("status"=>1,"msg"=>"Registered Successfully","user_type"=>"trucker"); 
