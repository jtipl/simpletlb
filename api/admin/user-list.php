...
$sel = $Global->db->prepare("SELECT count(*) as allcount FROM admin_users");
$sel->execute();
$records =$sel->fetch(PDO::FETCH_ASSOC);
$totalRecords = $records['allcount'];

// Total number of record with filtering
$sel = $Global->db->prepare("SELECT count(*) as allcount FROM admin_users WHERE (user_name ::text ILIKE :searchQuery) ");
$sel->execute(array("searchQuery" => $searchQuery));
$records =$sel->fetch(PDO::FETCH_ASSOC);
$totalRecordwithFilter = $records['allcount'];

// Fetch records
$featurelist = $Global->db->prepare("SELECT id,user_name,user_type,email,password,to_char(admin_users.created_date,'MM-DD-YYYY') as created_date FROM admin_users WHERE (user_name ::text ILIKE :searchQuery) ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$rowperpage." OFFSET ".$row);
