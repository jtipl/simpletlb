...
$searchValue =    isset($_REQUEST['search']['value']) ? $_REQUEST['search']['value']: '';
		...
		if($operation=="trucker_current_excel"){
			...
			$sel = $Global->db->prepare("SELECT count(*) as allcount FROM users u JOIN trucker tr ON u.id = tr.user_id AND u.user_type='trucker'");
			$sel->execute();
			...
			$loadslist = $Global->db->prepare("SELECT u.name , u.email , tr.phone , u.status FROM users u INNER JOIN  trucker tr ON  
				u.id=tr.user_id  AND u.status=1
