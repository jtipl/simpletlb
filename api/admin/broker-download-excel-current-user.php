...
if($operation=="broker_current_excel"){
	// Total number of records without filtering
	$sel = $Global->db->prepare("SELECT count(*) as allcount FROM users u JOIN broker br ON u.id = br.user_id AND u.user_type='broker'");
	$sel->execute();
	...
	$loadslist = $Global->db->prepare("SELECT u.name , u.email , br.phone ,u.status FROM users u INNER JOIN  broker br ON  
		u.id=br.user_id  AND u.status=1
