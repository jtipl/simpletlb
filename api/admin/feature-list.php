...
$sel = $Global->db->prepare("SELECT count(*) as allcount FROM feature_list");
$sel->execute();
$records =$sel->fetch(PDO::FETCH_ASSOC);
$totalRecords = $records['allcount'];

// Total number of record with filtering
$sel = $Global->db->prepare("SELECT count(*) as allcount FROM feature_list WHERE (feature_name ::text ILIKE :searchQuery) ");
$sel->execute(array("searchQuery" => $searchQuery));
$records =$sel->fetch(PDO::FETCH_ASSOC);
$totalRecordwithFilter = $records['allcount'];

// Fetch records
$featurelist = $Global->db->prepare("SELECT id,feature_name,to_char(feature_list.created_date,'MM-DD-YYYY') as created_date,status,sub_menu,menu_url,is_report FROM feature_list WHERE (feature_name ::text ILIKE :searchQuery) ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$rowperpage." OFFSET ".$row);
