    ...
    $(".export_csv").click(function(){
      ...
      if(total_count!=0){
          ...
          if(export_page=="1"){
            ...
            var href=LoadBoard.API+"shipper/download-excel-current-view-loads?token="+token+"&operation=picked&user_id="+user_id+"&start="+start+"&length="+length;
            window.location.href=href;
          }
          else if(export_page=="2"){
            var href=LoadBoard.API+"shipper/download-excel-all-loads?token="+token+"&operation=picked&length=all&user_id="+user_id;
            window.location.href=href;
          } 
        } 
    });
  ...
  $(document).ready(function() {
      ...
      var table = $('#viewloads').DataTable({
         language: { search: "",searchPlaceholder: "Search for..." ,"zeroRecords": "No relevant information available" , "sInfo": " _START_ - _END_ of _TOTAL_ ", "infoFiltered": ""},
           dom: 'Bfrtip',
            "ajax": {
             url :LoadBoard.API+'shipper/in-progress',
             type:"post",
             headers: {
