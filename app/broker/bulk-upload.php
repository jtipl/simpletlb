...
$data_count = $Global->db->prepare("SELECT COUNT(*) FROM temp_loads WHERE status=0 AND reload_status = 1 AND user_id = ".$broker_user_id."");
$data_count->execute();
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    if(data_count != 0){
          swal.fire({
            ...
            if(result.value){
               window.location.href=LoadBoard.APP+'broker/view-bulk';
            }else{
              return false;
            }
        });
  ...
  function imageuplaod(){
          ...
          $.ajax({
              type: "POST",
              url: LoadBoard.API+'broker/bulk_api',
              data: form_data,
              contentType: false,
              cache: false,
              processData:false,
