...
$error_data = $Global->db->prepare("SELECT broker.phone,users.name ,users.email,users.business_name,tem.id, tem.user_id, tem.origin, tem.destination, tem.origin_address, tem.origin_city, tem.origin_state, tem.origin_country, tem.origin_zipcode, tem.destination_address, tem.destination_city, tem.destination_state, tem.destination_country, tem.destination_zipcode, to_char(tem.pickup_date,'MM-DD-YYYY') as pickup_date,  to_char(tem.delivery_date,'MM-DD-YYYY') as delivery_date, tem.pickup_time, tem.delivery_time, tem.truck_load_type, tem.weight, tem.length,height, tem.truck_name, tem.price, tem.created_date, tem.created_by, tem.app_type, tem.updated_date, tem.updated_by, tem.data_validate_reason, tem.data_validate_status, tem.reload_status, tem.status, tem.origin_bulk, tem.destination_bulk FROM temp_loads as tem INNER JOIN users ON users.id = tem.user_id INNER JOIN broker ON broker.user_id =users.id  WHERE users.user_type='broker' AND tem.data_validate_status = 0 AND tem.status=0 AND tem.reload_status = 1 AND tem.user_id = ".$broker_user_id."");
$error_data->execute();
$error_bulk_data = $error_data->fetchAll(PDO::FETCH_ASSOC);
$error_datas = json_encode($error_bulk_data) ;

$contact_inf = $Global->db->prepare("SELECT broker.phone,users.name ,users.email,users.business_name FROM  users INNER JOIN broker ON broker.user_id =users.id  WHERE users.user_type='broker' AND users.id = ".$broker_user_id."");
$contact_inf->execute();
$broker_contact = $contact_inf->fetch(PDO::FETCH_ASSOC);

//$br = json_encode($broker_contact) ;

//print_r($error_datas);exit;
$valid_data = $Global->db->prepare( "SELECT broker.phone,users.name ,users.email,users.business_name,tem.id, tem.user_id, tem.origin, tem.destination, tem.origin_address, tem.origin_city, tem.origin_state, tem.origin_country, tem.origin_zipcode, tem.destination_address, tem.destination_city, tem.destination_state, tem.destination_country, tem.destination_zipcode, to_char(tem.pickup_date,'MM-DD-YYYY') as pickup_date,  to_char(tem.delivery_date,'MM-DD-YYYY') as delivery_date, tem.pickup_time, tem.delivery_time, tem.truck_load_type, tem.weight, tem.length,height, tem.truck_name, tem.price, tem.created_date, tem.created_by, tem.app_type, tem.updated_date, tem.updated_by, tem.data_validate_reason, tem.data_validate_status, tem.reload_status, tem.status, tem.origin_bulk, tem.destination_bulk FROM temp_loads as tem INNER JOIN users ON users.id = tem.user_id INNER JOIN broker ON broker.user_id =users.id  WHERE users.user_type='broker' AND tem.data_validate_status = 1 AND tem.status=0 AND tem.reload_status = 1 AND tem.user_id = ".$broker_user_id."");
$valid_data->execute();
         ...
         <div class="col-md-12 col-sm-12">
            <div class="col-sm-4 col-md-4">
               <div class="form-group">
                  ...
                  <input type="text" id="broker_name" name="broker_name" class="form-control" placeholder="">
                  <label id="broker_name-error" class="broker_name" for="broker_name" style="display:none;"></label>
               </div>
            </div>
            <div class="col-sm-4 col-md-4">
               <div class="form-group">
                  <label class="form-label">Email</label>
                  <input type="text" id="broker_email" name="broker_email" class="form-control" placeholder="">
                  <label id="broker_name-error" class="broker_email" for="broker_email" style="display:none;"></label>
               </div>
            </div>
            <div class="col-sm-4 col-md-4">
               <div class="form-group">
                  <label class="form-label">Phone</label>
                  <input type="text" id="broker_phone" name="broker_phone" class="form-control" placeholder="">
                  <label id="broker_name-error" class="broker_phone" for="broker_phone" style="display:none;"></label>
               </div>
            </div>
         </div>
      </div>
...
<script type="text/javascript">
  $(document).ready(function(){
   /* var name=<?php echo($broker_contact['name']);?>;
    var phone=<?php echo($broker_contact['phone']);?>;
    var email=<?php echo($broker_contact['email']);?>;
    $("#broker_name").val(jsUcfirst(name));
    $("#broker_email").val(email);
    $("#broker_phone").val(formatPhoneNumber(phone));*/
...
if(value == '' || value==null){
     callback(false) ;
      ...
      $.ajax({
         type:'POST',
         url:LoadBoard.API+'broker/bulk-location-list',
...
afterValidate: function (isValid, value,row,prop, source) {
   ...
   if(source=='undefined' || source==undefined){
     ...
     $.ajax({
        url: LoadBoard.API+'broker/bulk-view-change',
        type:'post',
        data:datt,
...
function bulk_confirm(){
    $('.error').html();
    if($('#broker_name').val()=='' || $('#broker_email').val() =='' || $('#broker_phone').val()==''){
   ...
   //   console.log(data);
      $.ajax({
         ...
         type: 'POST',
        // async:false,
         data: {"data": data,"broker_name":$("#broker_name").val(),"broker_email":$("#broker_email").val(),"broker_phone":$("#broker_phone").val()}, //returns all cells' data
         dataType: 'json',
         beforeSend:function(){
           $(".preloader").show();
         },
...
$('#re_home').on('click',function(){
    swal.fire({
        ...
        if(result.value){
           window.location.href='/loadboard/app/broker/dashboard'
        }else{
          return false;
        }
    });
    ...
    swal.fire({
        ...
        if(result.value){
           $.ajax({
            ...
            data:{action:'delete'},
            success:function(res){
                 window.location.href='/loadboard/app/broker/bulk-upload';
            }
          });
        }else{
          return false;
