    ...
    <div class="nav lb-nav-tabss sm" id="nav-tab" role="tablist">
      ...
      <a class="nav-item nav-link active" id="nav-trucker-tab" data-toggle="tab" href="#nav-trucker" role="tab" aria-controls="nav-trucker" aria-selected="false"> 
        <i class="icon-DeliveredLoads"></i> &nbsp;
         Delivered Loads &nbsp;&nbsp;<span class="tool" style="font-weight: normal;" data-tip="The list of finished loads" tabindex="1" ><i class="fa fa-question-circle-o"></i></span></a>    
...
$(document).ready(function() {
       ...
       // alert(total_count);
        if (total_count != 0) {
            ...
            if (export_page == "1") {
                ...
                var length = '10';
                var href = LoadBoard.API + "broker/download-excel-current-view-loads?token=" + LoadBoard.token + "&operation=" + operation + "&user_id=" + user_id + "&start=" + start + "&length=" + length;
                window.location.href = href;
            } else if (export_page == "2") {
                var href = LoadBoard.API + "broker/download-excel-all-loads?token=" + LoadBoard.token + "&operation=" + operation + "&length=all&user_id=" + user_id;
                window.location.href = href;
            }
        }
    });
    ...
    var cancelloads = $('#proviewloads').DataTable({
        ...
        dom: 'Bfrtip',
        "ajax": {
            url: LoadBoard.API + 'broker/cancel-loads', //?user_id='+LoadBoard.userid,
            type: "post",
            headers: {
                Authorization: "Bearer " + LoadBoard.token
            },
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    if (row.favorite_status == 0) {
                        favclass = "star";
                        favcon = '<span class="tool" data-tip="Mark this trucker favorite" tabindex="1" ><a class="star btncursor favclick favorite_' + row.trucker_id + '" data-broker-id="' + row.broker_id + '" data-trucker-id="' + row.trucker_id + '"></a></span>';
                    } else if (row.favorite_status == 1) {
                        favclass = "star-fav";
                        favcon = '<span class="tool" data-tip="Remove from favorite list" tabindex="1" ><a class="star-fav btncursor favclick favorite_' + row.trucker_id + '" data-broker-id="' + row.broker_id + '" data-trucker-id="' + row.trucker_id + '"></a></span>';
                    } else {
                        favclass = "star";
                        favcon = '<span class="tool" data-tip="Mark this trucker favorite" tabindex="1" ><a class="star btncursor favclick favorite_' + row.trucker_id + '" data-broker-id="' + row.broker_id + '" data-trucker-id="' + row.trucker_id + '"></a></span>';
                    }
    ...
    $(document).on("click", "#nav-home-tab", function() {
        ...
        expiredloads.ajax.url(LoadBoard.API + 'broker/view-loads').load();
    });
    $(document).on("click", "#nav-profile-tab", function() {
        cancelloads.ajax.url(LoadBoard.API + 'broker/cancel-loads').load();

    });
    $(document).on("click", "#nav-trucker-tab", function() {
        delivered_table.ajax.url(LoadBoard.API + "broker/past-loads").load();
    });
    ...
    $(document).on("click", ".favclick", function() {
        ...
        $.ajax({
            type: 'post',
            url: LoadBoard.API + 'broker/favorite',
            dataType: "json",
            headers: {
                Authorization: "Bearer " + LoadBoard.token
            },
...
function vinnoformat() {
    $.ajax({
        ...
        success: function(result) {
            //alert("<option>"+result.data+"</option>");
            if (result.status == 1) {
                //console.log(result.data.length)
                // alert(result.data.length)
                for (i = 0; i < result.data.length; i++) {
                    //alert(result.data[i]["veh_id_no"]); 
                    //$("#vvv").val(result.data[i]["veh_id_no"]);
