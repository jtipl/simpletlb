    ...
    $(".export_csv").click(function(){
        ...
        $(".export").show();
          if(export_page=="1"){
            ...
            //alert(start);
            var length = '10';
            var href=LoadBoard.API+"broker/download-favorite-current-view-loads?token="+LoadBoard.token+"&operation=favorite&user_id="+user_id+"&start="+start+"&length="+length;
            window.location.href=href;
          }
          else if(export_page=="2"){
            var href=LoadBoard.API+"broker/download-favorite-all-loads?token="+LoadBoard.token+"&operation=favorite&length=all&user_id="+user_id;
            window.location.href=href;
          } 
        } 
    });
  ...
  $(document).ready(function() {
      ...
      var table = $('#viewloads').DataTable({
        language: { search: "",searchPlaceholder: "Search for...","zeroRecords": "No relevant information available", "sInfo": " _START_ - _END_ of _TOTAL_ ", "infoFiltered": ""},
          dom: 'Bfrtip',
              "ajax": {
               url:LoadBoard.API+'broker/trucker-list',
               type:"post",
               headers: {
