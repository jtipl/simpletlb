...
$(".select2").select2({
 ...
 width: '250px',
 ajax: {
 url: LoadBoard.API+"shipper/loads-data",
 dataType: 'json',
 //delay: 250,
    ...
    //Autocomplete places
     $('#autocomplete_search_destin').keypress(function (e) {
      ...
      searchtable=$('#viewloads').DataTable({
          ...
          dom: 'Bfrtip',
          "ajax": {
            url: LoadBoard.API+"shipper/loads-reports",
           "data": function(data){
     ...
     $('#search_loads').on('submit', function (e) {
          ...
          if(from_date!='' && to_date!=''){
               if(from_date > to_date){
                ...
                $("#to_date").css("border"," 1px solid rgba(0, 40, 100, 0.20 )");   
                $(".datecheck").hide();            
                searchtable.ajax.url(LoadBoard.API+"shipper/loads-reports").load();
              }  
          }else{
            $("#from_date").css("border","  1px solid rgba(0, 40, 100, 0.20 )");
            $("#to_date").css("border"," 1px solid rgba(0, 40, 100, 0.20 )");   
     ...
     $('#trucker_search_loads').on('submit', function (e) {
          ...
          if(from_date!='' && to_date!=''){
               if(from_date > to_date){
                ...
                $("#trucker_to_date").css("border"," 1px solid rgba(0, 40, 100, 0.20 )");   
                $(".trucker_datecheck").hide();            
                searchtable.ajax.url(LoadBoard.API+"shipper/trucker-reports").load();
  ...
  if(export_page=="1"){
    ...
    for(var i=1; i<=total_count; i++){
      ...
      if(tabsid=="loads-reports"){
        var href=LoadBoard.API+"shipper/download-excel-current-loads-report?token="+token+"&user_id="+user_id+"&start="+start+"&length="+length+"&loadid="+loadid+"&origin="+autocomplete_search_orgin+"&destination="+autocomplete_search_destin+"&weight="+weight+"&height="+height+"&length1="+length1+"&price="+price+"&equipment="+equipment+"&load_status="+load_status+"&datetype="+datetype+"&from_date="+from_date+"&to_date="+to_date;;
        window.location.href=href;
      } else if(tabsid=="trucker-reports"){
        var href=LoadBoard.API+"shipper/download-excel-current-truckerloads-reports?token="+token+"&length=all&user_id="+user_id+"&loadid="+loadid+"&trucker_datetype="+trucker_datetype+"&trucker_from_date="+trucker_from_date+"&trucker_to_date="+trucker_to_date+"&trucker_name="+trucker_name+"&business_name="+business_name+"&usdot="+usdot+"&trucker_load_status="+trucker_load_status+"&trucker_load_id="+trucker_load_id;
        window.location.href=href;
      }
    }
    else if(export_page=="2"){
      if(tabsid=="loads-reports"){
        var href=LoadBoard.API+"shipper/download-excel-all-loads-reports?token="+token+"&length=all&user_id="+user_id+"&loadid="+loadid+"&origin="+autocomplete_search_orgin+"&destination="+autocomplete_search_destin+"&weight="+weight+"&height="+height+"&length1="+length1+"&price="+price+"&equipment="+equipment+"&load_status="+load_status+"&datetype="+datetype+"&from_date="+from_date+"&to_date="+to_date;
        window.location.href=href;
      } else if(tabsid=="trucker-reports"){
        var href=LoadBoard.API+"shipper/download-excel-all-truckerloads-reports?token="+token+"&length=all&user_id="+user_id+"&loadid="+loadid+"&trucker_datetype="+trucker_datetype+"&trucker_from_date="+trucker_from_date+"&trucker_to_date="+trucker_to_date+"&trucker_name="+trucker_name+"&business_name="+business_name+"&usdot="+usdot+"&trucker_load_status="+trucker_load_status+"&trucker_load_id="+trucker_load_id;
        window.location.href=href;
      }
    } 
});
